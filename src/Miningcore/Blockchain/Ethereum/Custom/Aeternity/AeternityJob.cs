...
public class AeternityJob : EthereumJob
{
    protected CortexCuckooCycle aeternityHasher;
    protected Sha3_256 sha3Hasher;
    protected Blake2b blake2bHasher;

    public AeternityJob(string id, EthereumBlockTemplate blockTemplate, ILogger logger, IEthashLight ethash) : base(id, blockTemplate, logger, ethash)
    {
        this.aeternityHasher = new CortexCuckooCycle();
    ...
    {
        using(var stream = new MemoryStream(AeternityConstants.CuckarooHeaderNonceSize))
        {
            var bw = new BinaryWriter(stream);

            bw.Write(BlockTemplate.Header.HexToByteArray());
            bw.Write((BitConverter.IsLittleEndian ? BitConverter.GetBytes(nonce) : BitConverter.GetBytes(nonce).ReverseInPlace())); // aeternity-cuckoo-cycles expects a little endian format.
    ...
    protected virtual uint[] SerializeSolution(string solution)
    {
        ...
        var solutionUints = new uint[AeternityConstants.CuckarooSolutionSize];
        ...
        string workerName, string fullNonceHex, string solution, CancellationToken ct)
    {
        ...
        await Task.Run( () =>
        {
            ...
            if(aeternityHasher.Verify(headerBytes, solutionBytes) > 0)
                throw new StratumException(StratumError.MinusOne, "bad hash");
        }, ct);

        var resultBytes = SerializeCoinbase(solutionBytes);
        var blockTarget = new uint256(BlockTemplate.Target.HexToByteArray());
