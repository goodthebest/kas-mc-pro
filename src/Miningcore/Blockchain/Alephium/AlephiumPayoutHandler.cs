    ...
    IPayoutHandler
{
    ...
    public virtual async Task<Block[]> ClassifyBlocksAsync(IMiningPool pool, Block[] blocks, CancellationToken ct)
    {
        ...
        int minConfirmations = extraPoolPaymentProcessingConfig?.MinimumConfirmations ?? (network == "mainnet" ? 420 : 30);
        ...
        for(var i = 0; i < pageCount; i++)
        {
            ...
            for(var j = 0; j < page.Length; j++)
            {
                ...
                // 1) ALPH default lock mechanism: All of the mined coins are locked for N minutes (up to +8 hours on mainnet)
                // 2) Mining pool operator provides a custom minimum confirmations
                if(extraPoolPaymentProcessingConfig?.MinimumConfirmations == null)
                {
                ...
                else
                {
                    ...
                    var chainInfo = await Guard(() => alephiumClient.GetBlockflowChainInfoAsync(blockInfo.ChainFrom, blockInfo.ChainTo, ct),
                        ex=> logger.Debug(ex));
                    
                    block.ConfirmationProgress = Math.Min(1.0d, (double) (chainInfo.CurrentHeight - block.BlockHeight) / minConfirmations);
                }
    ...
    {
        return difficulty * AlephiumConstants.Pow2xDiff1TargetNumZero * AlephiumConstants.ShareMultiplier;
    }

    public double AdjustBlockEffort(double effort)
    {
        return effort * AlephiumConstants.Pow2xDiff1TargetNumZero * AlephiumConstants.ShareMultiplier;
    }
